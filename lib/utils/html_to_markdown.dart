String htmlToMarkdown(String html) {
  ...
  md = md.replaceAllMapped(RegExp(r'<h1[^>]*>(.*?)</h1>'), (m) => '# ${m[1]}\n\n');
  md = md.replaceAllMapped(RegExp(r'<h2[^>]*>(.*?)</h2>'), (m) => '## ${m[1]}\n\n');
  md = md.replaceAllMapped(RegExp(r'<h3[^>]*>(.*?)</h3>'), (m) => '### ${m[1]}\n\n');
  md = md.replaceAllMapped(RegExp(r'<strong>(.*?)</strong>'), (m) => '**${m[1]}**');
  ...
  md = md.replaceAllMapped(RegExp(r'<a[^>]+href="([^"]*)"[^>]*>(.*?)</a>'), (m) => '[${m[2]}](${m[1]})');
  md = md.replaceAllMapped(RegExp(r'<li>(.*?)</li>'), (m) => '- ${m[1]}\n');
  md = md.replaceAll(RegExp(r'</?[uo]l[^>]*>'), '\n');
  md = md.replaceAllMapped(RegExp(r'<blockquote>(.*?)</blockquote>', dotAll: true), (m) => '> ${m[1]}\n');
